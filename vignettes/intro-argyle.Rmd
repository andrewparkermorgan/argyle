...
* `attr(,"filter.sites")` -- character vector of length equal to number of *rows* of genotype matrix, with non-blank values flagging suspicious markers
* `attr(,"filter.samples")` -- character vector of length equal to number of *columns* of genotype matrix, with non-blank values flagging suspicious samples
...
```{r}
infile <- system.file("inst/extdata/wild.chr19.bed", package = "argyle")
...
ex <- run.sample.qc(ex, max.N = 500, max.H = list(inbred = 100, F1 = 10e3))
qcplot(ex)
```
...
| code | description | samples | markers |
|------|-------------|:-------:|:-------:|
| `N` | excess no-calls | **x** | **x** |
| `H` | excess heterozygosity | **x** | **x** |
| `I` | aberrant intensity pattern | **x** |  |
| `F` | aberrant allele frequency |  | **x** |
...
```{r, eval=FALSE, warning=FALSE, fig.width = 6, fig.height = 3.5}
...
```{r}
ff <- system.file("inst/extdata/wild.chr19.bed", package = "argyle")
wild <- read.plink(ff)
